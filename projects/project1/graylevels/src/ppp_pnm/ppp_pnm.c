 ...
 */
static int ppp_pnm_read_data(FILE *fd, enum pnm_kind kind, int rows, int columns, int maxval,
                             uint8_t *image, off_t offset, size_t length) {
    int i, q, column;
 ...
 */
extern uint8_t *ppp_pnm_read(const char *name, enum pnm_kind *kind, int *rows, int *columns,
                             int *maxval) {
    return ppp_pnm_read_part(name, kind, rows, columns, maxval, read_all);
}

extern uint8_t *ppp_pnm_read_part(const char *name, enum pnm_kind *kind, int *rows, int *columns,
                                  int *maxval,
    ...
    if (fd != NULL) {
        if (ppp_pnm_params(fd, kind, rows, columns, maxval) == 0) {
            if (*maxval < 256) {
                ...
                if (image != NULL) {
                    if (ppp_pnm_read_data(fd, *kind, *rows, *columns, *maxval, image, offset,
                                          length) == 0) {
                        fclose(fd);
                        return image;
                    }
 ...
 */
extern int ppp_pnm_write(const char *name, enum pnm_kind kind, int rows, int cols, int maxval,
                         uint8_t *image) {
    FILE *fd;
